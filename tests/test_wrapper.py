...
def test_finite_automata_read():
    ...
    transitions_automata = [(origin, symbol, target) for (origin, symbol), target in automata.transition_map.items()]
...
def test_finite_automata_write():
    ...
    with open("tests/examples/even_chars.fa") as input, open("tests/output/even_chars.fa") as output:
...
def test_nd_automata_write():
    ...
    with open("tests/examples/ends_with_bb.ndfa", "r") as input, open("tests/output/ends_with_bb.ndfa", "r") as output:
        assert input.readline() == output.readline()
...
def test_nd_automata_epsilon_write():
    ...
    with open("tests/examples/abc.ndfa", "r") as input, open("tests/output/abc.ndfa", "r") as output:
        assert input.readline() == output.readline()
